  ...
  <div class="g-bd">
    <r-title title="节目排行榜" big>
      <span class="sub-title">
        最近更新：{{ formatDate(updateTime, "MM月DD日") }}
      </span>
    </r-title>
    <ul>
      <li class="zebra-li" v-for="item in toplist" :key="item.id">
        ...
        <a class="row1 name">{{ item.program.name }}</a>
        <a class="row1 name2">{{ item.program.radio.name }}</a>
        <a class="category">{{ item.program.radio.category }}</a>
      </li>
    </ul>
  </div>
</template>
